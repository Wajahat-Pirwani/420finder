<template>
     <div class="review-wrapper" v-cloak>
        ...
        <ProductReviewDialog v-if="showLoginDialog" @hide-modal="hideModal" />
                ...
                <div class="customer-review-item" v-for="item in reviewCustomers" :key="item.id">
                    <div class="customer-review-head">
                        <div class="customer-review-head-img">
                            <img src="https://420finder.net/assets/img/logo.png" alt="User Profile" v-if="item.customer_profile == 'NULL'">
                ...
                <div class="pagination" style="background: #fff" v-if="pagination.total > 50">
                              ...
                              <button
                               ...
                               :class="{'grey_bg': changeButtonBg(pagination.current_page, pagination.current_page, 'prev')}"
                                @click="getProductReviews(pagination.current_page - 1)">
                        ...
                        <select name="" id="" @change="getProductReviews($event.target.value)" class="form-control">
        ...
        <template v-else>
            <div>
                <p class="p-bold" style="margin: 2rem;font-weight: bold; font-size: 1rem; line-height: 25px; text-align: center;">
                THIS PRODUCT HAS NO REVIEWS YET. BE THE FIRST ONE TO POST A REVIEW!
                <br>
                <a href="#" class="btn btn-primary" style="background: #F8971C; !important; border-color: #F8971C; margin-top: 1rem;" @click.prevent="checkReviewForm">
                    WRITE REVIEW
                </a>
               </p>
            </div>
...
import ProductReviewDialog from '../dialog/ProductReviewDialog.vue';

export default {
    props: ['retailerid', 'productid', 'user', 'isreviewed', 'businesstype'],
    components: {
    StarRating,
    ProductReviewDialog,
    },
    ...
    methods: {
        async getProductReviews(page = 1) {
            ...
            await axios.get(`/api/menu/get-product-reviews/${this.retailerId}/${this.productid}/${this.businessType}?page=${page}`).then((response) => {
